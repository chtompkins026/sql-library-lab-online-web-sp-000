...
def select_character_names_and_number_of_books_they_are_in
  "SELECT charachters.names, COUNT(charachters_books.id) FROM charachters LEFT JOIN charachters_books ON charachters.id = charachters_books.charachter_id GROUP_BY(charachters.name) ORDER BY(COUNT(charachters_books.id)) DESC LIMIT 1"
end
