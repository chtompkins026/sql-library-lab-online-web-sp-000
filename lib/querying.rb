...
def select_series_title_with_most_human_characters
  "SELECT series.title FROM series JOIN authors ON series.author_id = authors.id JOIN charachters ON authors.id = charachters.authors_id WHERE charachters.species = "human" ORDER BY (COUNT(characters.species)"
end
